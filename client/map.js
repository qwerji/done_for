const GameStateManager = (function() {
    ...
    function getFreshWorldMap(argument) {
        return {
            'Ship': {
    ...
    publicMethods.refresh = function() {
        worldMap = getFreshWorldMap()
        attributes = getFreshAttributes()
    }

    publicMethods.getSituation = function(hero) {
        return filter(worldMap[hero.location], hero)
    }

    publicMethods.filter = function(situation, hero) {
        return filter(situation, hero)
    }

    publicMethods.changeAttr = function(option) {
        changeAttr(option)
    }
